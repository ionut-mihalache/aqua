...
*/
void printServiceResponse(
    const struct eCAL_SServiceResponse *service_response) {
    ...
    printf("Response  : %.*s\n", (int)service_response->response_length,
           (char *)service_response->response);
...
/*
  Callback function that will be executed when we receive a response from a
  server.
*/
void serviceResponseCallback(
    const struct eCAL_SServiceResponse *service_response, void *user_data) {
    (void)user_data;
...
int main() {
    ...
    */
    ...
    printf("eCAL %s (%s)\n", eCAL_GetVersionString(),
           eCAL_GetVersionDateString());
    eCAL_Process_SetState(eCAL_Process_eSeverity_healthy,
                          eCAL_Process_eSeverityLevel_level1,
                          "Waiting for a service ...");

    /*
      Create a ServiceMethodInformation struct that contains the information
      about the service methods we want to call. In this case, we want to call
      the methods "echo" and "reverse".
    */
    method_information_set =
        (struct eCAL_SServiceMethodInformation *)eCAL_Malloc(
            2 * sizeof(struct eCAL_SServiceMethodInformation));
    if (method_information_set == NULL) {
        printf("Memory allocation failed.\n");
        return -1;
    }

    memset(method_information_set, 0,
           2 * sizeof(struct eCAL_SServiceMethodInformation));
    ...
    */
    mirror_client =
        eCAL_ServiceClient_New("mirror", method_information_set, 0, NULL);
    ...
    */
    ...
    /*
      Now that we are connected, we can set the process state to "healthy" and
      communicate the connection.
    */
    eCAL_Process_SetState(eCAL_Process_eSeverity_healthy,
                          eCAL_Process_eSeverityLevel_level1, "Connected!");
    ...
    */
    ...
    while (eCAL_Ok()) {
        ...
        */
        ...
        struct eCAL_ClientInstance **client_instances =
            eCAL_ServiceClient_GetClientInstances(mirror_client);
        ...
        /*
          ...
          With this approach we have the option to filter out client instances
          that we don't want to call. If you want to call either way all
          instances, then you can use
        ...
        */
        ...
        if (client_instances != NULL) {
            for (size_t j = 0; client_instances[j] != NULL; ++j) {
                /*
                  ...
                  We leave the default timeout value (infinite) for the blocking
                  call. You can change this for a specified timeout in ms.
                */
                struct eCAL_SServiceResponse *response =
                    eCAL_ClientInstance_CallWithResponse(client_instances[j],
                                                         method_name, request,
                                                         strlen(request), NULL);
                if (response != NULL) {
                ...
                /*
                  ...
                  The callback will be executed when the server has processed
                  the request and sent a response. You can again set a timeout
                  value for an internal waiting time. By default, we wait
                  infinitely.
                */
                if (eCAL_ClientInstance_CallWithCallback(
                        client_instances[j], method_name, request,
                        strlen(request), serviceResponseCallback, NULL,
                        NULL) != 0) {
        ...
        /*
          Now we set the process state according to the result of the service
          calls. You will see the state in the eCAL Monitor or the eCAL Sys
          application.
        */
        if (calls_ok != 0) {
            eCAL_Process_SetState(eCAL_Process_eSeverity_healthy,
            ...
            eCAL_Process_SetState(eCAL_Process_eSeverity_critical,
                                  eCAL_Process_eSeverityLevel_level3,
                                  "Calls failed!");
        }
