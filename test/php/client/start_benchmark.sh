runAQUAClientBenchmark() {
    qType=$1

    rm /dev/shm/*

    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/root/aqua/build/lib/shared

    cd /root/aqua/test/java/service/xslt-transformation
    make clean
    make
    make run PAYLOAD_TYPE=${qType} &
    aquaPid=$!
    sleep 2
    cd -

    for i in $(seq 1 9);
    do
        ./benchmark.sh false $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024
    done

    ./benchmark.sh true $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024

    kill $aquaPid
}

runAQUAThroughputBenchmark() {
    qType=$1
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/root/aqua/build/lib/shared

    # msgNumbers=(1000 4000 7000 10000 13000 16000 19000 22000 25000 28000)

    rm /dev/shm/*

    cd /root/aqua/test/java/service/xslt-transformation
    make clean
    make
    make throughput-run PAYLOAD_TYPE=${qType} &
    aquaPid=$!
    sleep 2
    cd -

    for i in $(seq 1 9);
    do
        ./benchmark-throughput.sh false $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000
        # kill $aquaPid
    done

    # rm /dev/shm/*

    # cd /root/aqua/test/java/service/xslt-transformation
    # make clean
    # make
    # make throughput-run PAYLOAD_TYPE=${qType} &
    # aquaPid=$!
    # sleep 2
    # cd -

    ./benchmark-throughput.sh true $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000
    kill $aquaPid
}

runUDSClientBenchmark() {
    qType=$1

    cd /root/aqua/test/java/service/xslt-transformation/src/uds
    make clean
    make
    make run PAYLOAD_TYPE=${qType} &
    udsPid=$!
    sleep 2
    cd -

    cd ./uds

    for i in $(seq 1 9);
    do
        ./benchmark.sh false $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024
    done

    ./benchmark.sh true $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024

    cd -

    kill $udsPid
}

runUDSThroughputBenchmark() {
    qType=$1

    cd /root/aqua/test/java/service/xslt-transformation/src/uds
    make clean
    make
    make throughput-run PAYLOAD_TYPE=${qType} &
    udsPid=$!
    sleep 2
    cd -

    cd ./uds

    for i in $(seq 1 9);
    do
        ./benchmark-throughput.sh false $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000
        # kill $udsPid
    done

    # rm /dev/shm/*

    # cd /root/aqua/test/java/service/xslt-transformation/src/uds
    # make clean
    # make
    # make throughput-run PAYLOAD_TYPE=${qType} &
    # udsPid=$!
    # sleep 2
    # cd -

    ./benchmark-throughput.sh true $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000

    cd -

    kill $udsPid
}

runZeroMQClientBenchmark() {
    qType=$1

    cd /root/aqua/test/java/service/xslt-transformation/src/zeromq
    make clean
    make
    make run PAYLOAD_TYPE=${qType} &
    zeromqPid=$!
    sleep 2
    cd -

    cd ./zeromq

    for i in $(seq 1 9);
    do
        ./benchmark.sh false $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024
    done

    ./benchmark.sh true $qType /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) 128 256 384 512 640 758 886 1024

    cd -

    kill $zeromqPid
}

    qType=$1

runZeroMQThroughputBenchmark() {
    qType=$1

    cd /root/aqua/test/java/service/xslt-transformation/src/zeromq
    make clean
    make
    make throughput-run PAYLOAD_TYPE=${qType} &
    zeromqPid=$!
    sleep 2
    cd -

    cd ./zeromq

    for i in $(seq 1 9);
    do
        ./benchmark-throughput.sh false $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000
        # kill $zeromqPid
    done

    # rm /dev/shm/*

    # cd /root/aqua/test/java/service/xslt-transformation/src/zeromq
    # make clean
    # make
    # make throughput-run PAYLOAD_TYPE=${qType} &
    # zeromqPid=$!
    # sleep 2
    # cd -

    ./benchmark-throughput.sh true $qType $(pwd) 1000 4000 7000 10000 13000 16000 19000 22000 25000 28000

    cd -

    kill $zeromqPid
}

runClientBenchmark() {
    # qTypes=(SMB EMB QMB HMB MB DMB HGB GB)
    qTypes=(SMB EMB QMB HMB MB)

    for qType in "${qTypes[@]}";
    do
        runAQUAClientBenchmark ${qType}
        runUDSClientBenchmark ${qType}
        runZeroMQClientBenchmark ${qType}

        lastAQUAbenchmark=$(ls benchmark_results/clients/ | grep -o "[0-9]\+" | sort -rn | head -n 1)
        lastUDSbenchmark=$(ls uds/benchmark_results/clients/ | grep -o "[0-9]\+" | sort -rn | head -n 1)
        lastZMQbenchmark=$(ls zeromq/benchmark_results/clients/ | grep -o "[0-9]\+" | sort -rn | head -n 1)

        qTypeLower=${qType,,}
        metrics2Path="metrics2_${qTypeLower}"
        metrics3Path="metrics3_${qTypeLower}"

        if [ ! -d ${metrics2Path} ]; then
            mkdir -p ${metrics2Path}
        fi
        if [ ! -d ${metrics3Path} ]; then
            mkdir -p ${metrics3Path}
        fi

        cp benchmark_results/clients/$lastAQUAbenchmark/client_benchmark.csv ${metrics2Path}/aqua_client_benchmark.csv
        cp uds/benchmark_results/clients/$lastUDSbenchmark/client_benchmark.csv ${metrics2Path}/uds_client_benchmark.csv

        cp benchmark_results/clients/$lastAQUAbenchmark/client_benchmark.csv ${metrics3Path}/aqua_client_benchmark.csv
        cp uds/benchmark_results/clients/$lastUDSbenchmark/client_benchmark.csv ${metrics3Path}/uds_client_benchmark.csv
        cp zeromq/benchmark_results/clients/$lastZMQbenchmark/client_benchmark.csv ${metrics3Path}/zeromq_client_benchmark.csv

        source venv/bin/activate
        python3 create_combined_plots.py ${metrics2Path} benchmark_results/clients/$lastAQUAbenchmark/client_benchmark.csv uds/benchmark_results/clients/$lastUDSbenchmark/client_benchmark.csv
        python3 create_combined_plots.py ${metrics3Path} benchmark_results/clients/$lastAQUAbenchmark/client_benchmark.csv uds/benchmark_results/clients/$lastUDSbenchmark/client_benchmark.csv zeromq/benchmark_results/clients/$lastZMQbenchmark/client_benchmark.csv
        deactivate
    done
}

runThroughputBenchmark() {
    qTypes=(SMB EMB QMB HMB MB)
    # qTypes=(SMB)

    for qType in "${qTypes[@]}";
    do
        runAQUAThroughputBenchmark ${qType}
        runUDSThroughputBenchmark ${qType}
        runZeroMQThroughputBenchmark ${qType}

        lastAQUAbenchmark=$(ls benchmark_results/throughput/ | grep -o "[0-9]\+" | sort -rn | head -n 1)
        lastUDSbenchmark=$(ls uds/benchmark_results/throughput/ | grep -o "[0-9]\+" | sort -rn | head -n 1)
        lastZMQbenchmark=$(ls zeromq/benchmark_results/throughput/ | grep -o "[0-9]\+" | sort -rn | head -n 1)

        qTypeLower=${qType,,}
        metrics2Path="throughput_metrics2_${qTypeLower}"
        metrics3Path="throughput_metrics3_${qTypeLower}"

        if [ ! -d ${metrics2Path} ]; then
            mkdir -p ${metrics2Path}
        fi
        if [ ! -d ${metrics3Path} ]; then
            mkdir -p ${metrics3Path}
        fi

        cp benchmark_results/throughput/$lastAQUAbenchmark/throughput_benchmark.csv ${metrics2Path}/aqua_throughput_benchmark.csv
        cp uds/benchmark_results/throughput/$lastUDSbenchmark/throughput_benchmark.csv ${metrics2Path}/uds_throughput_benchmark.csv

        cp benchmark_results/throughput/$lastAQUAbenchmark/throughput_benchmark.csv ${metrics3Path}/aqua_throughput_benchmark.csv
        cp uds/benchmark_results/throughput/$lastUDSbenchmark/throughput_benchmark.csv ${metrics3Path}/uds_throughput_benchmark.csv
        cp zeromq/benchmark_results/throughput/$lastZMQbenchmark/throughput_benchmark.csv ${metrics3Path}/zeromq_throughput_benchmark.csv

        source venv/bin/activate
        python3 create_combined_plots_throughput.py ${metrics2Path} benchmark_results/throughput/$lastAQUAbenchmark/throughput_benchmark.csv uds/benchmark_results/throughput/$lastUDSbenchmark/throughput_benchmark.csv
        python3 create_combined_plots_throughput.py ${metrics3Path} benchmark_results/throughput/$lastAQUAbenchmark/throughput_benchmark.csv uds/benchmark_results/throughput/$lastUDSbenchmark/throughput_benchmark.csv zeromq/benchmark_results/throughput/$lastZMQbenchmark/throughput_benchmark.csv
        deactivate
    done
}

# qTypes=(SMB EMB QMB HMB MB DMB HGB GB)

# for qType in "${qTypes[@]}";
# do
#     runAQUAThroughputBenchmark $qType
# done

# runAQUAClientBenchmark SMB
# runAQUAThroughputBenchmark SMB

runClientBenchmark
runThroughputBenchmark
# runAQUAThroughputBenchmark QMB
# runUDSThroughputBenchmark QMB
# runZeroMQThroughputBenchmark QMB

# ./benchmark.sh /home/$(whoami)/FlameGraph /home/$(whoami)/async-profiler $(pwd) $(date +%s) 128 256 384 512 640 758 886 1024